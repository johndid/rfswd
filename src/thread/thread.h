...
///But do not remove any thing from the interface
///
///the class contains two functions that should be used from the code that want's to start and stop the thread,
...
///continue to run.
///
///If the object instance goes out of scope, the thread will automatically call stop (from it's destructor) and teh thread will be stopped
///
///To get the benefits from this class you'll have to derive a new class that implements the functionality that you need to have in a thread.
///To do this, simply, override the run method. Theres no need to create a while(1) statement inside your over written method in the derived class
///as the run() method is already inside a loop.
///
///However, The run() method returns a boolean, where false means that the thread has stopped, with out stop being called,
///returning true means keep on running!
///
