...
const bool posix_thread::start() 
{
  if(!running) {
    ...
    //    log::msg()->error("posix_thread::start -> failed - Couldn't start the thread");
  }
  return false;
}
...
void posix_thread::run_till_stop() 
{
  ...
  while(running) {
    run();
    //    ::pthread_yield(); //TODO: test to see if this makes the thing run faster!
  }
}
...
void posix_thread::dump_err(const int err) 
{
  switch(err) {
  case EDEADLK:
    //    log::msg()->error("posix_thread::dump_err -> EDEADLK");
    break;
  case EINVAL:
    //    log::msg()->error("posix_thread::dump_err -> EINVAL");
    break;
  case ESRCH:
    //    log::msg()->error("posix_thread::dump_err -> ESRCH");
    break;
  default:
    //    log::msg()->error("posix_thread::dump_err -> unknown");
    break;
  }
}
